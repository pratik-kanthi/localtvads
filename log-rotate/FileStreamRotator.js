 ...
 *
 * let rotatingLogStream = require('FileStreamRotator').getStream({filename:"/tmp/test.log", frequency:"daily", verbose: false})
...
FileStreamRotator.getFrequency = function (frequency) {
    const _f = frequency.toLowerCase().match(/^(\d+)([m|h])$/);
    if (_f) {
...
FileStreamRotator.getDate = function (format, date_format) {
    ...
    if (format && staticFrequency.indexOf(format.type) !== -1) {
        switch (format.type) {
        case 'm':
            const minute = Math.floor(moment().minutes() / format.digit) * format.digit;
            return moment().minutes(minute).format(date_format);
        case 'h':
            const hour = Math.floor(moment().hour() / format.digit) * format.digit;
    ...
    if (file.hash === crypto.createHash('md5').update(file.name + 'LOG_FILE' + file.date).digest('hex')) {
        try {
            if (fs.existsSync(file.name)) {
                ...
                if (fs.existsSync(gzippedFileName)) {
                    fs.unlinkSync(gzippedFileName);
                }
            }
        } catch (e) {
...
FileStreamRotator.getStream = function (options) {
    ...
    if (curDate && frequencyMetaData && staticFrequency.indexOf(frequencyMetaData.type) > -1) {
        ...
        stream.write = function (str, encoding) {
            const newDate = this.getDate(frequencyMetaData, dateFormat);

...
module.exports = (app) => {
    ...
    app.get('/api/clientplans/:id', passport.authenticate('jwt', { session: false }), async (req, res) => {
        try {
            ...
            const result = await ClientAdPlan.findOne({ Client: req.user.Claims[0].Value, _id: req.params.id }).populate('Channel AdVideo AddOnAssets').sort('BookedDate').lean().exec();
            return res.status(200).send(result);
        } catch (ex) {
            return res.status(ex.code || 500).send(ex.error);
        }
